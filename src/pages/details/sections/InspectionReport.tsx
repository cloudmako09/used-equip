...
const InspectionReport = (props: Props_InspectionReport) => {
    ...
    const itemCount = props.jsonDataProduct ? props.jsonDataProduct.condition.category.length : 0;
    ...
    const [openCloseStates, setOpenCloseStates] = useState<boolean[]>(initialOpenCloseStates);
    ...
    if (props.jsonDataProduct == null) { return <></>; }
    ...
    //inspection results
    if (props.jsonDataProduct.condition !== null && typeof props.jsonDataProduct.condition.category !== "undefined"
        && props.jsonDataProduct.condition.category !== null && typeof props.jsonDataProduct.condition.category[0] !== "undefined") {




        const inspectionReportHtml = props.jsonDataProduct.condition.category.map((curInsp, x) => {
            //for each category

            return (<div key={x} className="panel-group">
                <div className="panel panel-default">
                        <div className="panel-heading" onClick={()=>toggleOpen(x)}>
                        <h4 className="panel-title"> 
                            <span className="glyphicon glyphicon-plus" aria-hidden="true"></span> {curInsp.name} 
                        ...
                        <ul className="list-group">
                            {
                                curInsp.section.map((curSection, y) => {
                                    //for each section
                                    if (typeof curSection['section-detail'] != "undefined" && curSection['section-detail'] != null) {
                                        return curSection['section-detail'].map((curSectDet, z) => {
                                            return <InspectionSingleItem
                                                key={z}
                                                curItem={curSectDet}
                                                indexNum={z}
                                                lang={props.lang} />;
        ...
        return (
            <div id="details_specifications_wrapper" className="container-fluid"> <div className="container">
                <div id="inspection" className="row detailsection_specifications">

                    <div id="detailTabs" className="col-xs-12 col-lg-8 col-lg-offset-2 ">

                        <ul className="nav nav-tabs">
                            {/* <!--<li className="active"><a data-toggle="tab" href="#tab_features">Features</a></li>--> */}
                            <li className="active">
                                {outputEnFr("Inspection Report", "Voir le rapport d'inspection", props.lang)}
                            </li>
